# res://addons/ActionNet/scripts/ui/action_net_manager_ui.gd
extends Node
class_name ActionNetManagerUI
...
func _on_host_button_pressed():
	print("[ActionNetManagerUI] Host button pressed")
	var port = int(port_input.text)
	var result = await ActionNetManager.create_server(port)
	if result == OK:
		print("[ActionNetManagerUI] Server started successfully")        
		_start_client("127.0.0.1", port)
	else:
		print("[ActionNetManagerUI] Failed to start server")
...
func _on_join_button_pressed():
	...
	if not ip.is_valid_ip_address():
		...
		print("[ActionNetManagerUI] Attempting to resolve hostname: ", ip)
		var resolved_ip = IP.resolve_hostname(ip, IP.TYPE_ANY)
		if resolved_ip != "":
			ip = resolved_ip
			print("[ActionNetManagerUI] Resolved hostname to IP: ", ip)
		else:
			print("[ActionNetManagerUI] Failed to resolve hostname")
			return
...
func _on_create_server_button_pressed():
	...
	var result = await ActionNetManager.create_server(port)
	if result == OK:
		print("[ActionNetManagerUI] Server created successfully")
	else:
		print("[ActionNetManagerUI] Failed to create server")

func _start_client(ip: String, port: int):
	ActionNetManager.create_client(ip, port)

func _on_connected_to_server():
	print("[ActionNetManagerUI] Successfully connected to server")
	emit_signal("connected_to_server")

func _on_connection_failed():
	print("[ActionNetManagerUI] Failed to connect to server")
